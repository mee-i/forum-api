...
describe('RepliesRepository postgres', () => {
    ...
    describe('verifyReply function', () => {
        it('should throw error if reply not found', async () => {
            ...
            // Action & Assert
            await expect(repliesRepositoryPostgres.verifyReply('reply-123', 'comment-456'))
                .rejects
                .toThrowError(NotFoundError);
        });

        it('should not throw error if reply found', async () => {
            // Arrange
            await UsersTableTestHelper.addUser({ id: 'user-123' });
            await ThreadsTableTestHelper.addThread({ id: 'thread-123', owner: 'user-123' });
            await CommentsTableTestHelper.addComment({
            ...
            // Action & Assert
            const reply = await repliesRepositoryPostgres.verifyReply('reply-123', 'comment-123');
    ...
    describe('deleteReplyById function', () => {
        it('should throw error when reply not found', async () => {
            ...
            // Action
            await expect(repliesRepositoryPostgres.deleteReplyById('reply-123')).rejects.toThrowError(NotFoundError);
        });
